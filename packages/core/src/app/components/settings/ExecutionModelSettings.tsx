...
type ExecutionModelSettingsProps = {
  ...
  onSetActiveConnection: (id: string, scope?: 'session' | 'project-default') => Promise<void>;
};
    ...
    <div className="grid gap-5">
      ...
      {executionTab === 'local-cli' ? (
        <LocalCliPanel
          ...
          projectDefaultConnectionId={projectDefaultConnectionId}
          scanState={scanState}
  ...
  return (
    <section aria-labelledby="local-cli-heading" className="grid gap-4">
      ...
      {localConnections.length > 0 && (
        <div className="grid gap-2">
          <h4 className="text-[12.5px] font-semibold text-foreground">
            Configured Local Connections
          </h4>
          <div className="grid gap-2">
            {localConnections.map((conn) => {
              const isActive = conn.id === activeConnectionId;
              const isDefault = conn.id === projectDefaultConnectionId;
                ...
                >
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {isDefault && (
                        <span className="rounded bg-muted border border-border px-1.5 py-0.5 text-[10px] font-medium text-foreground">
                          Default
                        </span>
                      )}
                    </div>
                    <div className="flex flex-wrap items-center gap-1.5 mt-0.5 text-[11px] text-muted-foreground">
                      ...
                      {conn.agentCommandAlias && (
                        <>
                          <span>•</span>
                          <span>{conn.agentCommandAlias}</span>
                        </>
                      )}
                      {conn.lastTestedAt && (
                        <>
                    ...
                    {conn.status.state === 'ready' && conn.status.message && (
                      <div className="mt-1.5 flex flex-col gap-1 rounded-[6px] border border-green-200 bg-green-50/20 p-2 text-[11px] text-green-700">
                        <span className="font-semibold flex items-center gap-1">
                          <span className="inline-block size-1.5 rounded-full bg-green-500" />
                          Verification details:
                        </span>
                        <pre className="mt-0.5 max-h-24 overflow-y-auto font-mono text-[10px] whitespace-pre-wrap text-green-800 bg-green-50/40 p-1.5 rounded border border-green-100/50">
                          {conn.status.message}
                        </pre>
                      </div>
                    )}
                  ...
                  <div className="flex items-center gap-1.5">
                    {!isActive && (
                      <button
                        type="button"
                        onClick={() => onSetActiveConnection(conn.id, 'session')}
                    ...
                    {!isDefault && (
                      <button
                        type="button"
                        onClick={() => onSetActiveConnection(conn.id, 'project-default')}
                        className="inline-flex h-8 items-center justify-center rounded-[6px] border border-hairline bg-background px-2.5 text-[11px] font-medium text-foreground hover:bg-muted"
                      >
                        Set Default
                      </button>
      ...
      <div className="rounded-[8px] border border-hairline bg-muted/35 px-3 py-2 text-[12px] text-muted-foreground">
        Configured local connections: {localConnections.length}
      </div>
    </section>
  );
}
