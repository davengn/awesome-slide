---
title: Overview
description: How Awesome Slide and your coding agent work together.
---

Awesome Slide treats your coding agent as a first-class user. Decks are React,
agents are great at React, the gap between *"make slides about X"* and a
polished deck is mostly the runtime — which Awesome Slide provides.

## What ships in the workspace

`npx @awesome-slide/cli init` generates an `AGENTS.md` (and `CLAUDE.md`) at the
...
they version with the project. `awesome-slide sync:skills` re-syncs them from
...
- Don't touch `package.json`, `awesome-slide.config.ts`, or other slides.
...
scaffolder; edit `AGENTS.md` and re-run `awesome-slide sync:skills` if you
