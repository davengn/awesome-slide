...
`awesome-slide.config.ts`, or pass your own object that satisfies the `Locale`
type.

The locale only affects the **runtime UI** that Awesome Slide renders around
...
```ts title="awesome-slide.config.ts"
import type { OpenSlideConfig } from '@awesome-slide/core';
import { zhTW } from '@awesome-slide/core/locale';
...
All presets are re-exported from the `@awesome-slide/core/locale` subpath:

```ts
import { en, zhTW, zhCN, ja } from '@awesome-slide/core/locale';
```
...
`Locale` type from `@awesome-slide/core`.

```ts title="awesome-slide.config.ts"
import type { OpenSlideConfig, Locale } from '@awesome-slide/core';
import { en } from '@awesome-slide/core/locale';
