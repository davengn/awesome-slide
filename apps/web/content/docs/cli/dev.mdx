...
The skills check also obeys the `AWESOME_SLIDE_SKIP_SKILLS_CHECK=1`
environment variable — useful in CI or container builds where stdin
isn't a TTY.

## Configuration

Override the port (and a few other knobs) in `awesome-slide.config.ts`:
